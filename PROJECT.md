...
- Ingestion and processing are decoupled — `--skip-ingest` flag lets you run dedup → score → newsletter on existing `raw_deals.json` without re-fetching.
...
### Ideas
- Improve query to fetch good articles from scratch: using prompt engineering: Create a list of basic vocubulary to search for like FMCG industry names (ever-growing) and high quality deal terminologies or negative sampling by introducing feedback loop

---

## Filtering Layer (Dedup)
...
### Credibility Filter
- Articles are checked against `SOURCE_TIERS` (domain → score mapping) before LLM scoring. Articles below `CREDIBILITY_CUTOFF` are discarded entirely — they don't get keyword-scored as fallback.
- Mobile subdomains are normalized (m.economictimes.com → economictimes.com).
- Corroborated articles (appeared in 2+ sources after dedup) auto-pass regardless of domain score.

### Problems Encountered
- Generic articles without any industry merger but overall scene: Tried Scispacy (ORG, MONEY...) but not doing as good job as LLM
- Tried spaCy NER for deal value extraction — unreliable, grabbed first MONEY entity regardless of context (e.g. "$2 trillion" from unrelated sentence). Removed in favor of LLM-extracted `deal_value_structured`.
...
### Approach
- Multi-format output: Markdown, Word (.docx), and styled HTML
- LLM generates narrative sections (headline deal, sector pulse, executive summary) with template fallbacks when LLM is unavailable
- HTML version deployed to GitHub Pages via GitHub Actions

---

## Solved Problems
- The LLM call was sequential for all articles, made it parallel using asyncio and OpenAI async .ainvoke() with a semaphore (max 5 concurrent) to avoid rate limiting
- LLM calls getting stuck: Added a 60s timeout per call — falls back to keyword scoring on timeout
- Input data was taking long time to run: Made it parallel using concurrent threads (ThreadPoolExecutor, 6 workers) since trafilatura was I/O bound
- Non-credible articles were inflating the newsletter with low-quality keyword-scored entries: Now discarded entirely
- Cost visibility: Added a cost tracker that accumulates token usage and cost (from OpenRouter metadata) across all LLM calls, prints summary at end of run

---

## Future Ideas
- Better query engineering for fetching higher quality articles
- Feedback loop for improving source quality over time
- BM25 as alternative/complement to TF-IDF for dedup
- Sentence embedding model for semantic dedup at scale

---

## Learnings
- Content hashing is used for exact word to word easy duplicate handling.
- The crossover point: MinHash+LSH beats brute force at roughly 10K+ documents. Below that, the index construction overhead (building hash tables, banding, bucketing) costs more than just computing all pairwise similarities directly.
